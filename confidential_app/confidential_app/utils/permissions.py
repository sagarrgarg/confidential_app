"""
Permission layer for the Confidential App.

Provides has_permission hooks, permission_query_conditions, caching
helpers, and audit logging used by hooks.py to restrict access to
...
def _is_enabled():
    """Return whether the confidential system is active (cached for 30 s)."""
    now = time()
    if now - _enabled_cache["ts"] < _ENABLED_TTL:
        return _enabled_cache["val"]
    try:
        result = _is_protection_enabled_uncached()
    except Exception:
        result = True
    _enabled_cache.update(ts=now, val=result)
    return result
...
def _is_protection_enabled_uncached():
    """Uncached version — hits the DB / settings."""
    try:
        from confidential_app.confidential_app.doctype.confidential_settings.confidential_settings import (
            get_settings,
        )
...
def _is_admin(user):
    ...
    return bool(user_roles & set(ADMIN_ROLES))
...
def _user_has_doc_access(doctype, doc_name, user):
    """Core access check: role-level or user-level access to a confidential doc."""
    user_roles = set(frappe.get_roles(user))
...
def _check_sub_bom_confidentiality(bom_name, user, checked=None):
    """Recursively check if any sub-BOM is confidential and user lacks access."""
    if checked is None:
        checked = set()
...
def has_doctype_permission(doctype, doc, user=None, ptype=None, **_kwargs):
    """Base permission check function for confidential doctypes.

    Args:
        doctype: "BOM", "Stock Entry", or "Work Order"
        doc: Document instance or document name string
        user: Optional user email
        ptype: Permission type passed by Frappe (read/write/create/…)

    Returns:
        True  – explicitly allow (admin or has required role/user)
    ...
    if ptype == "create":
        if isinstance(doc, str) or (hasattr(doc, "is_new") and doc.is_new()):
            return None
    ...
    if isinstance(doc, str) and doc:
        doc_name = doc
    elif doc is not None and hasattr(doc, "name") and doc.name:
        doc_name = doc.name

    if not doc_name:
        return None

    # Always read is_confidential from DB (the saved state) so that an
    # in-flight save that toggles the flag doesn't block itself.
    try:
...
# ---------------------------------------------------------------------------
# has_permission hook wrappers (registered in hooks.py)
# Frappe passes ptype= as a keyword arg; we accept it and forward.
# ---------------------------------------------------------------------------

def has_bom_permission(doc, user=None, ptype=None, **kwargs):
    """has_permission hook for BOM."""
    return has_doctype_permission("BOM", doc, user, ptype)


def has_stock_entry_permission(doc, user=None, ptype=None, **kwargs):
    """has_permission hook for Stock Entry (basic)."""
    return has_doctype_permission("Stock Entry", doc, user, ptype)


def has_work_order_permission(doc, user=None, ptype=None, **kwargs):
    """has_permission hook for Work Order."""
    return has_doctype_permission("Work Order", doc, user, ptype)


def has_stock_entry_submit_permission(doc, user=None, ptype=None, **kwargs):
    """Enhanced has_permission hook for Stock Entry.
    ...
    try:
        ...
        if doc.bom_no and doc.purpose in (
            "Manufacture",
        ...
        return has_stock_entry_permission(doc, user, ptype)
...
# ---------------------------------------------------------------------------
# List view filtering (permission_query_conditions hooks)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Cached BOM permission check (used by bom_override & validations)
# ---------------------------------------------------------------------------

_permission_cache = {}
_cache_timeout = PERMISSION_CACHE_TIMEOUT


def clear_permission_cache():
    """Clear the in-process BOM permission cache."""
    global _permission_cache
    _permission_cache = {}
    debug_log("Permission cache cleared")


def invalidate_bom_cache(bom_name):
    """Remove all cache entries for a specific BOM (call on confidentiality change)."""
    global _permission_cache
    _permission_cache = {
        k: v for k, v in _permission_cache.items()
        if not k.startswith(f"bom_perm:{bom_name}:")
    }
...
def check_bom_permission(bom):
    ...
    Uses per-process caching (TTL = PERMISSION_CACHE_TIMEOUT).
    """
    if not _is_enabled():
        return True
    ...
    if not is_confidential:
        ...
        result = _user_has_doc_access("BOM", bom, user)
